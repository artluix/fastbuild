...
static const FString FASTBuild_CachePath(TEXT("D:\\Fastbuild\\Cache\\Shaders"));
static const FString FASTBuild_ExecutableRootPath(TEXT("D:\\")); // needed to specify FASTBuild root of the virtual system simulation
...
bool FShaderCompileFASTBuildThreadRunnable::IsSupported()
{
	...
	// Check if the FASTBuild executable file exists
	IPlatformFile& platformFile = FPlatformFileManager::Get().GetPlatformFile();
	FString executablePath = FPaths::Combine(FPaths::EngineDir(), TEXT("Binaries"), TEXT("ThirdParty"), TEXT("FastBuild"), TEXT("FBuild.exe"));
	if (!platformFile.FileExists(*executablePath))
	{
		FASTBuildShaderCompilerVariables::Enabled = 0;
		return false;
	}

	FASTBuild_ConsolePath = executablePath;
	return true;
}
...
/*
*  Generate .bff file header information, customize Compiler information, dependent file list
*/

void FShaderCompileFASTBuildThreadRunnable::WriteScriptFileHeader(FArchive* ScriptFile, const FString& WorkerName)
{
	...
	FString headerString = FString::Printf(
		...
		*FASTBuild_CachePath,
		*WorkerName,
		//*IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*FPaths::RootDir()) // we wan't to grac SDK sources too but SDK isn't placed inside UE4 structure
		*FASTBuild_ExecutableRootPath);

	ScriptFile->Serialize((void*)StringCast<ANSICHAR>(*headerString, headerString.Len()).Get(), sizeof(ANSICHAR) * headerString.Len());

	// Custom enumerator class, add the enumerated files to the ExtraFiles list
	...
	// Enumerate all files in the dependent directory of the platform toolchain
	// So that any clean remote Windows system can run ShaderCompilerWorker.exe smoothly
	...
	// Enumerate all ShaderWorker related files
	...
	// Enumerate all shaders according to the directory list where the shaders are located
...
int32 FShaderCompileFASTBuildThreadRunnable::CompilingLoop()
{
	...
	else
	{
		...
		if (BuildDelayElapsed && HasJobsToRun && ShaderBatchesInFlight.Num() == ShaderBatchesInFlightCompleted)
		{
			...
			// Generate All Targets information
